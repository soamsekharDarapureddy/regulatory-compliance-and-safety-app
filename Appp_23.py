...
# === MODIFICATION: Refined Header with Layout Adjustments ===
logo_col, title_col = st.columns([1.5, 5]) # Adjusted ratio to reduce gap
with logo_col:
    logo_path = "people_tech_logo.png"
    if os.path.exists(logo_path):
        st.image(logo_path, width=180) # Increased logo size for clarity
    else:
        st.error("Logo file not found. Ensure 'people_tech_logo.png' is present.")
with title_col:
    # Adjusted padding and font size for better balance
    st.markdown("""
        <div style="padding-top: 25px;">
          <h1 style="color:var(--accent); font-size:2.6em; margin:0; line-height:1.2;">Regulatory Compliance and Safety Verification Tool</h1>
        </div>
    """, unsafe_allow_html=True)
st.markdown("---")
...
COMPONENT_KNOWLEDGE_BASE = {
    ...
    "lm7805": {"subsystem": "General", "part_name": "Positive Voltage Regulator", "manufacturer": "Texas Instruments", "type": "Linear Regulator", "output_voltage": "5V", "package": "TO-220", "package_type": "Through-Hole"},
    "irfz44n": {"subsystem": "General", "part_name": "N-Channel Power MOSFET", "manufacturer": "Infineon", "type": "MOSFET", "drain_source_voltage_vdss": "55V", "package": "TO-220AB", "package_type": "Through-Hole"},
    "stm32f407": {"subsystem": "General", "part_name": "ARM Cortex-M4 MCU", "manufacturer": "STMicroelectronics", "type": "MCU", "flash_memory": "1MB", "package": "LQFP144", "package_type": "SMD"},
}
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    for row in table[1:]: # Skip header row
                        if len(row) >= 4:
                            test_data = {
                                "TestName": (row[0] or "").replace('\n', ' '),
                                "Expected": (row[1] or "").replace('\n', ' '),
                                "Actual": (row[2] or "").replace('\n', ' '),
                                "Result": (str(row[3]).upper() if row[3] else "N/A").replace('\n', ' '),
                                "Observations": (row[12] if len(row) > 4 else "").replace('\n', ' ')
                            }
